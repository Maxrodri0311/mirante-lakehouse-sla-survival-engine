...
if not exist "data\lakehouse\silver\fact_pipeline_execution" (
    echo [1/3] Bootstrapping Calibrated Lakehouse Telemetry (2,500 runs)...
    python src/data_generator.py --runs 2500
    if %ERRORLEVEL% NEQ 0 (echo [ERROR] Data generator failed && exit /b %ERRORLEVEL%)
) else (
    echo [1/3] Lakehouse Silver telemetry partition already present.
)

echo.
echo [2/3] Running Automated Pytest Suite...
python -m pytest tests/ -v
if %ERRORLEVEL% NEQ 0 (echo [ERROR] Pytest suite failed && exit /b %ERRORLEVEL%)

echo.
echo [3/3] Running Quantitative Latency Benchmarks (50 iterations)...
